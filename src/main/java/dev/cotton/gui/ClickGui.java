...
/**
 * Cotton Visuals – Click GUI
 *
 * Layout matches the HTML v6 prototype:
 *  ┌──────────────────────────────────────────────────────┐
 *  │ Sidebar (175px)  │  Main panel                       │
 *  │  Logo            │  Header (title + search)          │
 *  │  Nav categories  │  Module grid (2 columns)          │
 *  │  Footer (user)   │  Style switcher bar               │
 *  └──────────────────────────────────────────────────────┘
 */
public class ClickGui extends Screen {
    ...
    // Expanded cards (for bind/settings dropdown)
    private Module expandedModule = null;

    // Hovered module for tooltip
    private Module hoveredModule = null;

    // Style: 0=liquid, 1=classic, 2=custom
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        // Semi-transparent dark overlay over game world
    ...
    private void drawSidebar(DrawContext ctx, int x, int y, int mx, int my) {
        ...
        // Cotton cloud icon (simple SVG-like drawn shapes)
        ...
        ny = drawNavItem(ctx, x, ny, sw, "⚙", "Settings", Module.Category.SETTINGS, mx, my);
    ...
    private int drawNavItem(DrawContext ctx, int x, int y, int sw,
        ...
        int textColor = active ? TEXT_PRIMARY : (hovered ? TEXT_SECONDARY : TEXT_DIM);
        text(ctx, icon + "  " + label, x + 14, y + 6, textColor);
    ...
    private void drawMainPanel(DrawContext ctx, int px, int py, int pw, int mx, int my) {
        ...
        border(ctx, sx, py + 9, pw - 130 - 10 + pw - sx, 23, BORDER_SUBTLE);
        text(ctx, "⌕  " + (searchQuery.isEmpty() ? "Поиск..." : searchQuery),
                sx + 6, py + 15,
                searchQuery.isEmpty() ? TEXT_DIM : TEXT_PRIMARY);
        ...
        // Enable scissor to not draw below footer
        ...
        for (int i = 0; i < mods.size(); i++) {
            ...
            if (cy + CARD_H > maxY) break; // out of visible area

            boolean hovered = mx >= cx && mx <= cx + colW && my >= cy && my <= cy + CARD_H;
            if (hovered) hoveredModule = mod;

            moduleCard(ctx, cx, cy, colW, CARD_H, mod.isEnabled(), mod.isPinned(), hovered);
            drawModuleCard(ctx, mod, cx, cy, colW, CARD_H, hovered);
        }
    ...
    private void drawModuleCard(DrawContext ctx, Module mod, int x, int y, int w, int h,
        ...
        // Short description
        String desc = truncate(mod.getDescription(), 22);
        text(ctx, desc, x + 8, y + 18, TEXT_DIM);

        // Toggle switch
        toggle(ctx, x + w - 36, y + (h - 15) / 2, on);

        // PIN button (small star icon)
    ...
    private void drawStyleBar(DrawContext ctx, int px, int py, int pw) {
        ...
        text(ctx, "Стиль:", px + 10, py + 13, TEXT_DIM);
    ...
    // TOOLTIP
    // ────────────────────────────────────────────────────────────────────────
    private void drawTooltip(DrawContext ctx, Module mod, int mx, int my) {
        String desc = mod.getDescription();
        int tw = client.textRenderer.getWidth(desc);
        int tx = Math.max(4, Math.min(mx - tw / 2, this.width - tw - 8));
        int ty = guiY - 28;
    ...
    private void drawCottonIcon(DrawContext ctx, int x, int y) {
        // Cloud shape built from overlapping circles (rectangles as approximation)
        int c1 = color(0xFF, 0xA7, 0x8B, 0xFA); // light purple
        int c2 = color(0xFF, 0x60, 0xA5, 0xFA); // blue
        // Bottom blob
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        ...
        for (Module.Category cat : cats) {
            if (cat == Module.Category.WORLD)    ny += 15; // group gap
            if (cat == Module.Category.SETTINGS) ny += 15;
            if (mx >= x + 6 && mx <= x + SIDEBAR_W - 6 &&
                    my >= ny && my <= ny + NAV_ROW_H) {
                selectedCat = cat;
                return true;
            }
            ny += NAV_ROW_H + 2;
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (keyCode == 256 /* ESCAPE */ || keyCode == 344 /* RIGHT_SHIFT */) {
            this.close();
            return true;
        }
        // Search: backspace
